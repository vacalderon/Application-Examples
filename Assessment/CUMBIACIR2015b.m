...
name = 'Design_CL_5_00' ;     %identifies actual work, the output file will be name.xls
...
L             = 12000;                    % member clear length (mm)
bending       = 'single';                % single or double
ductilitymode = 'uniaxial';              % biaxial or uniaxial

% reinforcement details:

nbl     = 48;                % number of longitudinal bars
Dbl     = 39;             % long. bar diameter (mm)   
Dh      = 20;              % diameter of transverse reinf. (mm)
type    = 'spirals';        % 'spirals' or 'hoops'*
s       = 75;             % spacing of transverse steel (mm)*

% aplieed loads:

P      =  8310;                  % axial load kN (-) tension (+)compression
...
Ec      = 0;                 % concrete modulus of elasticity (MPa) or
                             ...
                             % 5000(fpc)^0.5
eco     = 0.0027;            % unconfined strain (usually 0.002 for normal weight or 0.004 for lightweight)*
esm     = 0.12;              % max transv. steel strain (usually ~0.10-0.15)*
espall  = 0.0064;            % max uncon. conc. strain (usually 0.0064)

fy      = 404.25;               % long steel yielding stress (MPa)
fyh     = 420;               % transverse steel yielding stress (MPa)
Es      = 200000;            % steel modulus of elasticity
fsu     = fy*1.35;               % long steel max stress (MPa)*
esh     = 0.004;             % long steel strain for strain hardening (usually 0.008)*
esu     = 0.11;              % long. steel maximum strain (usually ~0.10-0.15)*

Ey     =  700;                % slope of the yield plateau (MPa)
C1     =  3.3;                % defines strain hardening curve in the Raynor model [2-6]
...
ecdam = 'twth';     esdam = 0.045;   % concrete (ecser) and steel (esser) damage control strain
...
addpath('C:\ConditionDependentPBEE\Application-Examples\Assessment\models\')                  % directory with the user specified material models
...
err=0;
for i=1:nbl
    aux = find(yl>distld(i));
    conclay(aux(1),3) = conclay(aux(1),3)-Asb;
    if conclay(aux(1),3)<0
        err = err+1;
    end      
end
if err >0
...
for k=1:np
    ...
    while abs(F)>tol,
        ...
        if x(niter)>D
            eec = (def(k)/x(niter))*(x(niter)-D+conclay(:,1));
            ees = (def(k)/x(niter))*(x(niter)-D+distld);          % vector with the strains in the steel
        end
...
Mn     = interp1(coverstrain,mom,0.004);
esaux    = interp1(mom,steelstrain,Mn);
if esaux<-0.015
    Mn    = interp1(steelstrain,mom,-0.015);
end

cMn = interp1(mom,ejen,Mn);

fycurv = interp1(steelstrain,curv,-fy/Es);          % curvature for first yield
...
if criteria~=1 & (bucritMK == 1 & bucritBE==0) 
    buckldispl = interp1(CuDu,displ,failCuDuMK);
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        faildispl,failforce,'mo',buckldispl,bucklforce,'r*','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
    ...
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure',': buckling (M & K)');
end
if criteria~=1 & (bucritMK == 1 & bucritBE==1) 
    buckldispl = interp1(CuDu,displ,failCuDuMK);
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    buckldisplBE = interp1(CuDu,displ,failCuDuBE);
    bucklforceBE = interp1(CuDu,Force,failCuDuBE);
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        faildispl,failforce,'mo',buckldispl,bucklforce,'r*',buckldisplBE,bucklforceBE,'rs','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12); grid on;
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure',': buckling (M & K)',': buckling (B & E)');
end
if criteria~=1 & (bucritMK == 0 & bucritBE==1) 
    buckldisplBE = interp1(CuDu,displ,failCuDuBE);
    bucklforceBE = interp1(CuDu,Force,failCuDuBE);
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        faildispl,failforce,'mo',buckldisplBE,bucklforceBE,'rs','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12); grid on;
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure',': buckling (B & E)');
end
if criteria~=1 & (bucritMK == 0 & bucritBE==0)
    figure;plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        faildispl,failforce,'mo','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12);grid on;
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure');
...
if criteria==1 & (bucritMK == 1 & bucritBE==0)
    ...
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        buckldispl,bucklforce,'r*','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12); grid on;
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': buckling (M & K)');
end
if criteria==1 & (bucritMK == 1 & bucritBE==1) 
    buckldispl = interp1(CuDu,displ,failCuDuMK);
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    ...
    bucklforceBE = interp1(CuDu,Force,failCuDuBE);
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        buckldispl,bucklforce,'r*',buckldisplBE,bucklforceBE,'rs','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12); grid on;
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': buckling (M & K)',': buckling (B & E)');
end
if criteria==1 & (bucritMK == 0 & bucritBE==1) 
    buckldisplBE = interp1(CuDu,displ,failCuDuBE);
    bucklforceBE = interp1(CuDu,Force,failCuDuBE);
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        buckldisplBE,bucklforceBE,'rs','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12); grid on;
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': buckling (B & E)');
end
if criteria==1 & (bucritMK == 0 & bucritBE==0) 
    figure; plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Force - Displacement Relation','FontSize',16);
    legend(': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)');
end
...
if criteria~=1 & (bucritMK == 1 & bucritBE==0) 
    buckldispl = interp1(CuDu,displ,failCuDuMK);
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    figure;area(displ,Force,'FaceColor',[0.4 0.8 0.6]);hold on;area(displ(pointsdam),Force(pointsdam),'FaceColor',[0.4 0.6 0.6]);
    area(displ(pointsser),Force(pointsser),'FaceColor',[0.4 0.4 0.6]);
    plot(displbilin,forcebilin,'b--',displ,Force,'k',displ,V,'r:',displ,Vd,'m:',...
        faildispl,failforce,'mo',buckldispl,bucklforce,'r*','LineWidth',2, 'MarkerEdgeColor','k',...
        'MarkerFaceColor','g',...
        'MarkerSize',12); hold off;grid on;set(gca,'Layer','top');
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure',': buckling (M & K)');
end
if criteria~=1 & (bucritMK == 1 & bucritBE==1) 
    buckldispl = interp1(CuDu,displ,failCuDuMK);
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure',': buckling (M & K)',': buckling (B & E)');
end
if criteria~=1 & (bucritMK == 0 & bucritBE==1) 
    buckldisplBE = interp1(CuDu,displ,failCuDuBE);
    bucklforceBE = interp1(CuDu,Force,failCuDuBE);
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure',': buckling (B & E)');
end
if criteria~=1 & (bucritMK == 0 & bucritBE==0)
    figure;area(displ,Force,'FaceColor',[0.4 0.8 0.6]);hold on;area(displ(pointsdam),Force(pointsdam),'FaceColor',[0.4 0.6 0.6]);
    area(displ(pointsser),Force(pointsser),'FaceColor',[0.4 0.4 0.6]); 
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': shear failure');
...
if criteria==1 & (bucritMK == 1 & bucritBE==0)
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': buckling (M & K)');
end
if criteria==1 & (bucritMK == 1 & bucritBE==1) 
    buckldispl = interp1(CuDu,displ,failCuDuMK);
    bucklforce = interp1(CuDu,Force,failCuDuMK);
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': buckling (M & K)',': buckling (B & E)');
end
if criteria==1 & (bucritMK == 0 & bucritBE==1) 
    buckldisplBE = interp1(CuDu,displ,failCuDuBE);
    bucklforceBE = interp1(CuDu,Force,failCuDuBE);
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)',': buckling (B & E)');
end
if criteria==1 & (bucritMK == 0 & bucritBE==0) 
    figure;area(displ,Force,'FaceColor',[0.4 0.8 0.6]);hold on;area(displ(pointsdam),Force(pointsdam),'FaceColor',[0.4 0.6 0.6]);
    area(displ(pointsser),Force(pointsser),'FaceColor',[0.4 0.4 0.6]);
    ...
    xlabel('Displacement(m)','FontSize',16); ylabel('Force (kN)','FontSize',16);
    title('Potential Deformation Limit States','FontSize',16);
    legend(': ultimate zone',': damage control zone', ': serviceability zone',': bilinear approximation',': total response',': shear capacity (assessment)',': shear capacity (design)');
end
...
Acore = 0.25*pi*(dcore^2);


PCY   = interp1(ec,fc,ecser)*(Acore-AsLong)+interp1(ecun,fcun,ecser)*(Agross-Acore)+AsLong*interp1(es,fs,ecser); % compression force for serviceability
PTY   = AsLong*interp1(es,fs,ecser);                                                                             % axial force for serviceability
...
control = min(csid,ssid);

PCid   = interp1(ec,fc,control)*(Acore-AsLong)+interp1(ecun,fcun,control)*(Agross-Acore)+AsLong*interp1(es,fs,control); % compression force for first yielding
PTid   = AsLong*interp1(es,fs,control);                                                                                 % axial force for first yielding


PP  = [-0.90*PTid:0.30*PTid:0 0.05*fpc*Agross:0.05*fpc*Agross:0.6*PCid 0.7*PCid 0.8*PCid 0.90*PCid];               % vector with axial loads for interaction diagram
nPP = length(PP);
...
for i=1:nPP
    ...
    if abs(esaux)>abs(ssid) | Mni(i)==NaN
        cr = 1; %steel control
        Mni(i)     = interp1(steelstrain,mom,-ssid);
    end
    if cr==0
        eci(i)  = csid;
        esi(i)  = esaux;
